 ...
 *     the identified thread has terminated
 * - returns os_resultUnavailable if
 *     os_threadWaitExit has already been called by another thread.
 * - returns os_resultFail if
 *     there is no record of the thread.
 *       ( including if identified thread has terminated,
 *         and os_threadWaitExit has already been called,
 *         and post thread termination cleanup has completed. )
 */
OS_API os_result
os_threadWaitExit(
    os_threadId threadId,
