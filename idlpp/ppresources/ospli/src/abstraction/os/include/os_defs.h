...
#if __freertos__ == 1
#include <../freertos/include/os_os_defs.h>
#elif __linux__ == 1
...
#include <../tizenrt/include/os_os_defs.h>
#else
#error Platform missing from os_defs.h list
#endif

/* FIXME: This has been moved down because __nonnull on Xilinx 2017.3
          (armr5-none-eabi-gcc for ARM 6.2.1 20161205) does not like __nonnull
          to be redefined. It results in an error when stdlib.h is included. */
...
#if !defined(OS_HAVE_FILE_SYSTEM)
/* The OS_HAVE_FILE_SYSTEM is intended to inform upper layers if a file system
   is available for the target platform. For now availability is assumed if not
   defined in the target specific os_defs.h */
#define OS_HAVE_FILE_SYSTEM 1
#endif
