...
#include <assert.h>
#include <unistd.h>
#if ! defined PIKEOS_POSIX && ! defined __TIZENRT__
#include <strings.h>
#endif
#include <string.h>
#include <stdio.h>
    ...
    void *threadContext)
{
    ...
    strncpy (name, context.threadName, sizeof (name));
    name[127] = '\0';
    os_free (context.threadName);
    os_free (threadContext);

#ifdef INTEGRITY
    SetTaskName(CurrentTask(), name, strlen (name));
#else
    ...
    pthread_setspecific (os_threadNameKey, name);
...
#if !defined (OS_RTEMS_DEFS_H) && !defined (PIKEOS_POSIX) && !defined (__TIZENRT__)
