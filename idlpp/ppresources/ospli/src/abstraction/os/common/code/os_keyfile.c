...
static os_int32 os_keyfile_compareNameWithSocketFileAction(void *o, os_iterActionArg arg)
{
            ...
            if (strncmp(entry->d_name, "spddskey_", 9) == 0) {
                ...
                if (strncmp(entry->d_name, "osplsock_", 9) == 0) {
                    ...
                    if (pcKeyName == NULL) {
                        ...
                        socketNamesIter = os_iterAppend(socketNamesIter, (void*)strdup(entry->d_name));
                    }
                }
            }
            entry = readdir(key_dir);
