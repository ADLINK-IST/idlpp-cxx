    ...
    void *userData)
{
    ...
    if (idl_getLanguage() == IDL_LANG_LITE_CXX) {
        if (idl_getIsISOCpp())
        {
            snprintf(tmplFileName, (size_t)sizeof(tmplFileName), "%s%cliteISOCxxClassBody", tmplPath, OS_FILESEPCHAR);
        }
        else
        {
            snprintf(tmplFileName, (size_t)sizeof(tmplFileName), "%s%cliteCxxClassBody", tmplPath, OS_FILESEPCHAR);
        }
    } else {
