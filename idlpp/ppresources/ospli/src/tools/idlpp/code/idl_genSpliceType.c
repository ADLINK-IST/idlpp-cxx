    ...
    void *userData)
{
    ...
    for (i = 0; i < idl_depLength(idl_depDefGet()); i++) {
        ...
        if (idl_getIsISOCpp2()) {
            idl_fileOutPrintf(idl_fileCur(), "#include \"%s%s\"\n", idl_depGet (idl_depDefGet(), i), test_mode ? "Type.h" : "_DCPS.hpp");
        } else {
    ...
    void *userData)
{
    ...
    if (idl_scopeStackSize(scope) == 0 || idl_scopeElementType (idl_scopeCur(scope)) == idl_tModule) {
        ...
        idl_fileOutPrintf(idl_fileCur(), "extern const c_ulong %s_metaDescriptorArrLength;\n",fullyScopedName);
        idl_fileOutPrintf(idl_fileCur(), "extern const c_ulong %s_metaDescriptorLength;\n",fullyScopedName);
