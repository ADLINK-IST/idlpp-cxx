...
#include "idl_genSpliceLiteISOCxxType.h"
#include "idl_genLiteISOCxxHeader.h"
...
#include <Windows.h>
    ...
    const char* QUOTE = "";
#endif
    const char* IDLPP_CMD_OPTIONS = "a:P:d:o:b:m:t:hCSTD:I:l:j:n:iNx:";
    ...
    char *name)
{
    ...
    printf("Usage: %s [-b ORB-template-path]\n"
           ...
           "       [-l (isocpp | isoc++)] [-j [old]:<new>] [-d directory] [-i] \n"
#ifdef ENABLE_DEPRECATED_OPTION_DDS_TYPES
    ...
    char *name)
{
    ...
    printf(
        "    -l (isocpp | isoc++)\n"
        "       Defines the target language. isoc++ is an alias for isocpp.\n");
    printf(
...
OPENSPLICE_MAIN (lite_idlc)
{
    ...
    while ((opt = getopt(argc, argv, IDLPP_CMD_OPTIONS)) != -1) {
        ...
        case 'l':
            ...
            if ((strcmp(optarg, "isoc++") == 0) ||
                (strcmp(optarg, "isocpp") == 0) ){
        ...
        */
        ...
        if (makeAll) {
           ...
           if (idl_getLanguage() == IDL_LANG_CXX) {
               ...
               idl_fileOutPrintf(idl_fileCur(), "#include \"%s-cyclone.h\"\n", basename);

               idl_genSpliceLiteISOCxxTypeSetIncludeFileName(tmp);
               os_free(tmp);
               if (idl_getIsISOCpp())
               {
                   idl_walk(base, filename, source, traceWalk, idl_genSpliceLiteISOCxxTypeProgram());
               }
               else
               {
               ...
               if (idl_getIsISOCpp()) {
                   idl_walk(base, filename, source, traceWalk, idl_genLiteISOCxxHeaderProgram(&cxxUserData));
               } else {
                   idl_walk(base, filename, source, traceWalk, idl_genCorbaCxxCcppProgram());
               }
               idl_fileOutFree(idl_fileCur());
               ...
               if(idl_getIsISOCpp())
               {
                   ...
                   */
                   ...
                   idl_fileOutFree(idl_fileCur());
               } else if (idl_getCorbaMode() == IDL_MODE_STANDALONE) {
                   /* Call cppgen for both the user provided IDL file (filename),
                    * and the generated IDL file (fname).
                    */
                   char cpp_command[MAX_CPP_COMMAND];
                ...
                */
                ...
                idl_fileOutPrintf(idl_fileCur(), "#include \"os.h\"\n");
                idl_fileOutPrintf(idl_fileCur(), "#include \"%sSplDcps.h\"\n", basename);
