    ...
    void *userData)
{
    ...
    if (idl_typeSpecHasRef(idl_typeSpec(structSpec))) {
        ...
        idl_fileOutPrintf(
            idl_fileCur(),
            "    return (%s *)DDS_alloc(sizeof(%s), %s__free);\n",
            idl_scopeStack(scope, "_", name),
            idl_scopeStack(scope, "_", name),
            idl_scopeStack(scope, "_", name));
        struct_hasRef = TRUE;
        ...
        idl_fileOutPrintf(
            idl_fileCur(),
            "    return (%s *)DDS_alloc(sizeof(%s), NULL);\n",
            idl_scopeStack(scope, "_", name),
            idl_scopeStack(scope, "_", name));
        struct_hasRef = FALSE;
        action = idl_abort;
    ...
    if (idl_typeSpecHasRef(typeSpec)) {
	    ...
	    /* case 6148 */
            ...
            idl_fileOutPrintf(idl_fileCur(), "    }\n");
                ...
                idl_fileOutPrintf(
                    idl_fileCur(),
                    "    DDS_sequence_clean ((_DDS_sequence)&o->%s);\n",
                    name);
        }
    }
}
    ...
    void *userData)
{
    if (idl_typeSpecHasRef(typeSpec)) {
        if ((idl_typeSpecType(typeSpec) == idl_tstruct) ||
            ...
            idl_fileOutPrintf(
                idl_fileCur(),
                "        DDS_sequence_clean ((_DDS_sequence)&o->_u.%s);\n",
                name);
            idl_fileOutPrintf(idl_fileCur(), "        break;\n");
        } else {
            idl_fileOutPrintf (idl_fileCur(), "        break;\n");
    ...
    c_long indent)
{
    ...
    case idl_tseq:
        ...
        idl_fileOutPrintf(
            idl_fileCur(),
            "    DDS_sequence_clean ((_DDS_sequence)&%s",
            identifier);
        idl_arrayLoopRemoveIndex(typeArray);
        idl_fileOutPrintf (idl_fileCur(), ");\n");
	break;
    ...
    if ((idl_typeSpecType(idl_typeDefRefered(defSpec)) == idl_tstruct) ||
        ...
        if (idl_typeSpecType(idl_typeDefRefered(defSpec)) == idl_tbasic &&
            ...
            if (idl_typeSpecHasRef(idl_typeDefRefered(defSpec))) {
                ...
                idl_fileOutPrintf(
                    idl_fileCur(),
                    "    return (%s_slice *)DDS_alloc(sizeof(%s), %s__free);\n",
                    idl_scopeStack(scope, "_", name),
                    idl_scopeStack(scope, "_", name),
                    idl_scopeStack(scope, "_", name));
            } else {
                idl_fileOutPrintf(
                    idl_fileCur(),
                    "    return (%s_slice *)DDS_alloc(sizeof(%s), NULL);\n",
                    idl_scopeStack(scope, "_", name),
                    idl_scopeStack(scope, "_", name));
            }
            idl_fileOutPrintf(idl_fileCur(), "}\n");
    ...
    if (createFunctions) {
        ...
        if (idl_typeSpecHasRef(idl_typeSeqType(typeSeq))) {
            ...
            if (idl_typeSpecType(idl_typeSeqType(typeSeq)) == idl_tseq) {
                ...
                idl_fileOutPrintf(
                        idl_fileCur(),
                        "        DDS_sequence_free ((_DDS_sequence)&b[i]);\n");
                idl_fileOutPrintf(idl_fileCur(), "    }\n");
            } else {
                if ((idl_typeSpecType(idl_typeSeqType(typeSeq)) == idl_tbasic) ||
