    ...
    void *userData)
{
    ...
    snprintf(tmplFileName, sizeof(tmplFileName), "%s%cliteISOCxxMainInclude", tmplPath, OS_FILESEPCHAR);
    ...
    snprintf(tmplFileName, sizeof(tmplFileName), "%s%cliteISOCxxClassBody", tmplPath, OS_FILESEPCHAR);
    ...
    void *userData)
{
    ...
    if (macro)
    {
        ...
        idl_fileOutPrintf(idl_fileCur(), "#endif /* ISOCPP_%s_H */\n", baseName);
    }
}
    ...
    void *userData)
{
    ...
    if (idl_keyResolve(idl_keyDefDefGet(), idl_typeUserScope(idl_typeUser(structSpec)), name) != NULL)
    {
        ...
        const char *keyList = (strlen(internalTypeName) == 0) ?
                idl_keyResolve(idl_keyDefDefGet(), scope, name) : "key.localId,key.systemId";
 ...
 */
idl_program
idl_genLiteISOCxxHeaderProgram (
    void *userData)
{
