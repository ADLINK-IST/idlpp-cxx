    ...
    idl_typeSeq typeSeq)
{
    ...
    if (idl_typeSpecType(nextType) == idl_tunion ) {
        result = TRUE;
    }
    ...
    if (idl_typeSpecType(nextType) == idl_ttypedef) {
        ...
        if (idl_typeSpecType(alias) == idl_tunion ||
                idl_typeSpecType(alias) == idl_tarray ||
    ...
    os_boolean stacRequested)
{
    ...
    c_char *cxxTypeName = idl_ISOCxx2TypeFromTypeSpec(idl_typeSpec(typeBasic));
    ...
    void *userData)
{
    ...
    if (idl_typeSpecType(typeSpec) == idl_tbasic) {
        ...
        stacRequested = idl_stacDef_isStacDefined(scope, cxxIdName, typeSpec, NULL);
        ...
        } else
        {
            ...
            if ((idl_typeSpecType(idl_typeDefActual(idl_typeDef(typeSpec))) == idl_tstruct) ||
                ...
                c_char *cxxTypeName = idl_ISOCxx2TypeFromTypeSpec(typeSpec);
                idl_fileOutPrintf(idl_fileCur(), "    {\n");
                idl_fileOutPrintf(idl_fileCur(), "        extern void __%s__copyOut(const void *, void *);\n",
                        functionPrefix);
                idl_fileOutPrintf(idl_fileCur(), "        %s &tmp = to->%s();\n",
                    cxxTypeName,
                    cxxIdName);
                idl_fileOutPrintf(idl_fileCur(), "        __%s__copyOut((const void *)&from->%s, (void *)&tmp);\n",
                    functionPrefix,
                    cxxIdName);
                idl_fileOutPrintf(idl_fileCur(), "    }\n");
                os_free(cxxTypeName);
                os_free(functionPrefix);
            } else {
        ...
        idl_fileOutPrintf(idl_fileCur(), "        %s &tmp = to->%s();\n",
            cxxTypeName,
            cxxIdName);
        idl_fileOutPrintf(idl_fileCur(), "        __%s__copyOut((const void *)&from->%s, &tmp);\n",
            functionPrefix,
            cxxIdName);
        idl_fileOutPrintf(idl_fileCur(), "    }\n");
        os_free(cxxTypeName);
    ...
    } else if (idl_typeSpecType(typeSpec) == idl_tarray)
    {
        ...
        snprintf(destination, sizeof(destination), "to->%s()", cxxIdName);
        /* is a stac pragma defined for this member? */
        ...
        if(catsRequested)
        {
            ...
            snprintf(source, sizeof(source), "from->%s", cxxIdName);
        ...
        else if(stacRequested)
        {
            ...
            idl_arrayElements (idl_typeArray(typeSpec), "src", destination, 1, stacRequested, catsRequested);
            idl_fileOutPrintf (idl_fileCur(), "    }\n");
        }
        else
        {
            c_char *scopedDBType = idl_scopedSplTypeIdent(idl_typeArrayActual(idl_typeArray(typeSpec)));
            idl_fileOutPrintf(idl_fileCur(), "    {\n");
            idl_fileOutPrintf (idl_fileCur(), "        typedef %s _DestType", scopedDBType);
            idl_arrayDimensions(idl_typeArray(typeSpec), OS_FALSE);
            idl_fileOutPrintf(idl_fileCur(), ";\n");
            idl_fileOutPrintf(idl_fileCur(), "        const _DestType *src = &from->%s;\n\n", cxxIdName);
            idl_fileOutPrintf(idl_fileCur(), "        %s *dst = &to->%s();\n\n", cxxTypeName, cxxIdName);
            idl_arrayElements(idl_typeArray(typeSpec), "src", "*dst", 1, stacRequested, catsRequested);
            idl_fileOutPrintf(idl_fileCur(), "    }\n");
            os_free(scopedDBType);
        }
        /* QAC EXPECT 3416; No side effect here */
    } else if (idl_typeSpecType(typeSpec) == idl_tseq) {
        c_char type_name[256];
        idl_typeSpec nextType = idl_typeSeqType(idl_typeSeq(typeSpec));
        c_char *nextDBType = idl_scopedSplTypeIdent(nextType);
        c_char *toType = idl_ISOCxx2TypeFromTypeSpec(typeSpec);
        ...
        idl_fileOutPrintf(idl_fileCur(), "        size0 = c_arraySize(c_sequence(from->%s));\n",
            cxxIdName);
        if (idl_requiresInitializedSequence(idl_typeSeq(typeSpec))) {
            idl_fileOutPrintf(idl_fileCur(), "        to->%s().resize(size0);\n", cxxIdName);
        } else {
            idl_fileOutPrintf(idl_fileCur(), "        to->%s().clear();\n", cxxIdName);
            idl_fileOutPrintf(idl_fileCur(), "        to->%s().reserve(size0);\n", cxxIdName);
        }
    ...
    os_boolean catsRequested)
{
    ...
    case idl_tseq:
        ...
        dbTypeName = idl_scopedSplTypeIdent(nextType);
        ...
        idl_printIndent(total_indent);
        idl_fileOutPrintf(idl_fileCur(), "    %s.clear();\n", destin);
        idl_printIndent(total_indent);
        idl_fileOutPrintf(idl_fileCur(), "    %s.reserve(size0);\n", destin);
    ...
    c_long indent)
{
        ...
        if (idl_typeBasicType(idl_typeBasic(typeSpec)) == idl_string) {
            if(loop_index-1 > 0) {
                idl_fileOutPrintf(idl_fileCur(), "        i%dv.push_back(%s[i%d] ? %s[i%d] : \"\");\n",
                                  loop_index-1, from, loop_index-1, from, loop_index-1);
            } else {
                idl_fileOutPrintf(idl_fileCur(), "        (%s).push_back(%s[i%d] ? %s[i%d] : \"\");\n",
                                  to, from, loop_index-1, from, loop_index-1);
            }
        } else {
            if(loop_index-1 > 0) {
    ...
    case idl_tstruct:
    case idl_tunion:
        functionPrefix = idl_scopedTypeName(typeSpec);
        idl_fileOutPrintf(idl_fileCur(), "        extern void __%s__copyOut(const void *from, void *to);\n",
            functionPrefix);
        idl_printIndent(indent);
        idl_fileOutPrintf(idl_fileCur(), "        __%s__copyOut((const void *)&%s[i%d], (void *)&(%s)%s);\n",
            functionPrefix,
            from,
            loop_index-1,
            to,
    ...
    case idl_ttypedef:
        ...
        case idl_tbasic:
            /* QAC EXPECT 3416; No side effect here */
            if (idl_typeBasicType(idl_typeBasic(idl_typeDefActual(idl_typeDef(typeSpec)))) == idl_string) {
                idl_fileOutPrintf(idl_fileCur(), "        (%s)%s = %s[i%d] ? %s[i%d] : \"\";\n",
                    to,
                    seqIndex,
                    from,
                    loop_index-1,
        ...
        case idl_tseq:
            ...
            idl_fileOutPrintf(idl_fileCur(), "        size%d = c_arraySize(c_sequence(src%d));\n",
                ...
                loop_index);
            idl_printIndent(indent);
            if (idl_requiresInitializedSequence(idl_typeSeq(idl_typeDefActual(idl_typeDef(typeSpec))))) {
                idl_fileOutPrintf(idl_fileCur(), "        (%s)%s.resize(size%d);\n", to, seqIndex, loop_index);
            } else {
                idl_fileOutPrintf(idl_fileCur(), "        (%s)%s.clear();\n", to, seqIndex);
                idl_printIndent(indent);
                idl_fileOutPrintf(idl_fileCur(), "        (%s)%s.resize(size%d);\n", to, seqIndex, loop_index);
            }
    ...
    c_long indent)
{
    ...
    idl_fileOutPrintf(idl_fileCur(), "\n");
    idl_fileOutPrintf(idl_fileCur(), "    size%d = c_arraySize(c_sequence(src%d));\n", indent, indent);
    if (idl_requiresInitializedSequence(typeSeq)) {
        idl_fileOutPrintf(idl_fileCur(), "    (*to).resize(size%d);\n", indent);
    } else {
        idl_fileOutPrintf(idl_fileCur(), "    (*to).clear();\n");
...
idl_typedefOpenClose(
    ...
    void *userData)
{
    c_char *functionPrefix = idl_scopeStack(scope, "_", name);
    c_char *fwdFunctionPrefix = idl_scopedTypeName(idl_typeDefActual(defSpec));
    ...
    void *userData)
{
    ...
    if (idl_typeSpecType(typeSpec) == idl_tbasic) {
        /* QAC EXPECT 3416; No side effect here */
        if (idl_typeBasicType(idl_typeBasic(typeSpec)) == idl_string) {
            idl_fileOutPrintf(idl_fileCur(), "        to->%s(from->_u.%s ? from->_u.%s : \"\");\n",
                cxxIdName,
                cxxIdName,
                cxxIdName);
        } else {
        ...
        if (idl_typeSpecType(idl_typeDefRefered(idl_typeDef(typeSpec))) == idl_tarray) {
            ...
            idl_fileOutPrintf(idl_fileCur(), "            %s *x = new %s();\n",
                    cxxTypeName,
                    cxxTypeName);
            idl_fileOutPrintf(idl_fileCur(), "            __%s__copyOut((const void *)&from->_u.%s, (void *)x);\n",
                    functionPrefix,
                    cxxIdName);
            ...
            idl_fileOutPrintf(idl_fileCur(), "            %s x;", cxxTypeName);
            idl_fileOutPrintf(idl_fileCur(), "            __%s__copyOut((const void *)&from->_u.%s, (void *)&x);\n",
                    functionPrefix,
                    cxxIdName);
            idl_fileOutPrintf(idl_fileCur(), "            to->%s(x);\n", cxxIdName);
        ...
        c_char origin[256];
        c_char type_name[256];
        snprintf(origin, sizeof(origin), "&from->_u.%s", cxxIdName);
        snprintf(type_name, sizeof(type_name), "_%s", cxxIdName);
        idl_fileOutPrintf(idl_fileCur(), "    {\n");
        /* allocate the array on heap as it can have a huge size */
        idl_fileOutPrintf(idl_fileCur(), "        %s *x = new %s();\n\n",
            cxxTypeName,
        ...
        c_char type_name[256];
        idl_typeSpec nextType = idl_typeSeqType(idl_typeSeq(typeSpec));
        c_char *dbElementTypeName =  idl_scopedSplTypeName(nextType);
        ...
        idl_fileOutPrintf(idl_fileCur(), "        const %s *src0 = (const %s *)from->_u.%s;\n",
                ...
                dbElementTypeName,
                cxxIdName);
        /* TODO [EH]: Illegal, should be replaced with stackbased variable again. */
        idl_fileOutPrintf(idl_fileCur(), "        %s *x = new %s();\n\n",
            cxxTypeName,
            cxxTypeName);
        idl_fileOutPrintf(idl_fileCur(), "        size0 = c_arraySize(c_sequence(from->_u.%s));\n",
                cxxIdName);
        if (idl_requiresInitializedSequence(idl_typeSeq(typeSpec))) {
            idl_fileOutPrintf(idl_fileCur(), "        x->resize(size0);\n");
        } else {
            idl_fileOutPrintf(idl_fileCur(), "        x->clear();\n");
 ...
 */
idl_program
idl_genISOCxx2CopyoutProgram(void)
{
    return &idl_genISOCxx2Copyout;
}
