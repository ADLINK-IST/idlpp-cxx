...
static const c_ulong HEADERSIZE = ALIGNSIZE(C_SIZEOF(c_header));
static const c_ulong ARRAYHEADERSIZE = ALIGNSIZE(C_SIZEOF(c_arrayHeader));
    ...
    c_object object)
{
    ...
    assert(pa_ld32(&header->refCount) == 0);
#ifndef NDEBUG
    ...
    c_voidp o)
{
    ...
    assert(pa_ld32(&c_header(o)->refCount) > 0);
}
#endif
    ...
    c_object object)
{
    ...
    if ((safeCount = pa_dec32_nv(&header->refCount)) != 0)
    {
#if 0
        ...
        if (type && c_metaObject(type)->name) {
            if (strlen(c_metaObject(type)->name) >= CHECK_REF_TYPE_LEN) {
                if (strncmp(c_metaObject(type)->name, CHECK_REF_TYPE, CHECK_REF_TYPE_LEN) == 0) {
                    UT_TRACE("\n\n============ Keep(%p): %d -> %d =============\n",
                        object, pa_ld32(&header->refCount), pa_ld32(&header->refCount)+1);
                }
            }
        }
    }
    ...
    c_object object)
{
    ...
    return (c_long) pa_ld32(&c_header(object)->refCount);
}
